    ...
    public function run(Request $request, ScraperService $scraper, WhoisService $whois, PageSpeedService $pagespeed)
{
    try {
        ...
        if (!is_array($scraperResult) || ($scraperResult['status'] ?? 'success') === 'error') {
            ...
            try {
                $response = Http::timeout(300)->post('http://127.0.0.1:5000/extract', [
        ...
        $analysisData = [
            ...
            'pagespeed_metrics' => [],
            'pagespeed_scores' => [],
            // Initialiser les nouveaux champs PageSpeed par stratégie
        ...
        // 🚀 PageSpeed Insights - Job seulement
        ...
        // 🕒 CORRECTION CRITIQUE : Attendre que les jobs commencent
        Log::info('⏳ Attente de 5 secondes pour le démarrage des jobs...');
        sleep(10); // Attendre 10 secondes pour que les jobs démarrent
